...
def calculate_loss_weights(iteration, max_iterations, rmsd):
    ...
    # if rmsd > 6, bond and nonbonded weight is zero. 
    #if rmsd > 30.0:
    #    return torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)
    
    #rmsd = torch.clamp(rmsd, max=4.0) 
    # used to calculate scaling penalty for energy terms based on RMSD
    inv_rmsd = 1.0 / (3.0 ** rmsd)
...
def run_model(dat, xyz_data, xyz, model, indx, iterations, iteration, fixed_tors_ind, flexibility, curr_batch_size, f0_bonds,
    ...
    if (epoch <= 1):
        ...
        if loss_xyz > 3:
            ...
            energy = torch.tensor(0.0, device='cuda')
            print("loss_xyz > 20, use xyz loss only:")
            ...
            #loss = loss_xyz + non_bonded_energy + criterion(transformer_output, transformer_input)
            #loss = loss_xyz + bonded_energy + non_bonded_energy

            print("loss_xyz: ", loss_xyz.item())
            print("non_bonded_energy_loss: ", non_bonded_energy.item())
            print("loss_torsion: ", criterion(transformer_output, transformer_input).item())
...
if __name__ == '__main__':
    ...
    #top_path = "./traj/OLD_protein.prmtop"
    #path = "./traj/pepetide_min_rmrp.dcd"
