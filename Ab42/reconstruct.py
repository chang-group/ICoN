...
if __name__=='__main__':
    ...
    params_file_name = 'net_params' + name_list[9][5:]# name_list[Traj_ID][5:]

    
    print('Procesing Traj --', name_list[Traj_ID])
    ...
    #bondsP = va.pow(2).sum(dim=-1).sqrt()
    #bondsP = bondsP.clamp(min=0.96, max=1.96)
    ######################################################################
    z = z.detach().cpu().numpy()
    np.save('visual/' + name_list[Traj_ID] + 'latent' + '.npy', z)
    ...
    with mda.Writer('output/pdb/'+ name_list[Traj_ID] + '.dcd', n_atoms=u.atoms.n_atoms) as w:
        i = 0
